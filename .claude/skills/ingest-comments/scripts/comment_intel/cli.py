...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--skip-mirror", action="store_true", help="Do not copy skills to tool mirrors")
    parser.add_argument(
        "command",
        nargs="?",
        default="orchestrate",
        choices=("ingest", "refine", "generate", "orchestrate", "mirror"),
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command == "mirror":
        from .detect import git_root

        copied = mirror_skills(git_root(args.git_dir))
        print(json.dumps({"mirrored": copied}, indent=2))
        return 0
