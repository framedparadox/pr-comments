"""Git repository and GitHub identity detection."""
...
def parse_repo_slug(value: str, host: str = "github.com") -> tuple[str, str, str, str]:
    slug = value.strip().strip("/")
    if slug.endswith(".git"):
        slug = slug[:-4]
    if "github.com/" in slug:
        _, host, owner, repo = parse_remote_url(
            slug if slug.startswith("http") else f"https://{slug}"
        )
        return _, host, owner, repo
    parts = [p for p in slug.split("/") if p]
    if len(parts) < 2:
        raise SystemExit("expected owner/name (for example acme/app)")
    repo = parts[-1]
    owner = "/".join(parts[:-1])
    return provider_from_host(host), host, owner, repo
...
def resolve_base_branch(
    ...
    if identity.git_root:
        ...
        present = [
            ...
            if branch_exists(identity.git_root, name, runner=runner)
        ]
        if "main" in present:
            return "main"
