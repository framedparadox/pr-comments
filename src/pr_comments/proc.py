...
def parse_cli_json(text: str) -> Any:
    """Parse JSON, including `gh api --paginate` concatenated arrays (`][`)."""
    text = text.strip()
    if not text:
        return None
    try:
...
def http_get_json(url: str, headers: dict[str, str] | None = None, timeout: int = 60) -> tuple[Any, str | None]:
